...
if [ $INSTALL_NGINX = "True" ]; then
  ...
  server_name $WEBSITE_NAME $WEBSITE_NAME_WWW $IP_ADDRESS;
...
if [ $INSTALL_NGINX = "True" ] && [ $ENABLE_SSL = "True" ] && [ $ADMIN_EMAIL != "odoo@example.com" ]  && [ $WEBSITE_NAME != "_" ];then
  #sudo add-apt-repository ppa:certbot/certbot -y && sudo apt-get update -y (da error)
  sudo snap install --classic certbot
  sudo ln -s /snap/bin/certbot /usr/bin/certbot
  sudo apt-get install python3-certbot-nginx -y
  sudo certbot --nginx -d $WEBSITE_NAME,$WEBSITE_NAME_WWW --noninteractive --agree-tos --email $ADMIN_EMAIL --redirect
  sudo service nginx reload
